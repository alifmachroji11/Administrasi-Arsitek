  ...
  return (
    <svg width="20" height="20" viewBox="0 0 18 18" aria-hidden>
      ...
      <path
        ...
        d="M9 18c2.43 0 4.47-.8 5.96-2.18l-2.9-2.26c-.8.54-1.84.86-3.06.86-2.35 0-4.34-1.59-5.05-3.72H.9v2.33A9 9 0 0 0 9 18Z"
      />
      <path fill="#fff" d="M3.95 10.7A5.4 5.4 0 0 1 3.67 9c0-.59.1-1.17.28-1.7V4.97H.9A9 9 0 0 0 0 9c0 1.45.35 2.83.9 4.03l3.05-2.33Z" />
      <path
        fill="#fff"
        d="M9 3.58c1.32 0 2.51.46 3.44 1.35l2.58-2.58C13.46.89 11.43 0 9 0A9 9 0 0 0 .9 4.97l3.05 2.33C4.66 5.17 6.65 3.58 9 3.58Z"
      />
...
export function LandingPage({ googleEnabled }: { googleEnabled: boolean }) {
    ...
    <div ref={rootRef} className={styles.page}>
      ...
      <nav ref={navRef} className={styles.top}>
        ...
        <span className={styles.navActions}>
          <ThemeToggle />
          <TransitionLink className={styles.navLoginBtn} href="/login">
            <GoogleMark />
            Login
          </TransitionLink>
        </span>
      ...
      <section className={cx(styles.closing, styles.section)}>
        <div className={styles.wrap}>
          <div className={cx(styles.ctaBand, styles.reveal)}>
            <h2>Mulai dari proyek pertama kamu.</h2>
            <p>Masuk pakai akun Google — nggak perlu bikin password baru buat diinget.</p>
            {googleEnabled ? (
              <button type="button" className={styles.btnGoogleFilled} onClick={() => signIn("google", { callbackUrl: "/projects" })}>
                <GoogleMarkWhite />
                Lanjutkan dengan Google
              </button>
            ) : (
              <TransitionLink className={styles.btnGoogleFilled} href="/register">
                <GoogleMarkWhite />
                Lanjutkan dengan Google
              </TransitionLink>
            )}
